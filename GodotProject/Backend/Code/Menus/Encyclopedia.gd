...
func _ready():
	...
	if Wrong_path == true:
		$VBoxContainer/PharmingContainer/Wrong_path.disabled = false
		$VBoxContainer/PharmingContainer/Wrong_path/LockExample1.hide()
	if Malisiouse_prize == true:
		$VBoxContainer/PharmingContainer/Malisiouse_prize.disabled = false
		$VBoxContainer/PharmingContainer/Malisiouse_prize/LockExample1.hide()
	if Maliciouse_link == true:
		$VBoxContainer/PharmingContainer/Maliciouse_link.disabled = false
		$VBoxContainer/PharmingContainer/Maliciouse_link/LockExample1.hide()
	if Data_protection == true:
		$VBoxContainer/PhishihgContainer/Data_protection.disabled = false
		$VBoxContainer/PhishihgContainer/Data_protection/LockExample1.hide()
	if Maiciouse_email == true:
		$VBoxContainer/PhishihgContainer/Maiciouse_email.disabled = false
		$VBoxContainer/PhishihgContainer/Maiciouse_email/LockExample2.hide()
	if Maiciouse_email2 == true:
		$VBoxContainer/PhishihgContainer/Maiciouse_email2.disabled = false
		$VBoxContainer/PhishihgContainer/Maiciouse_email2/LockExample3.hide()
...
func _on_maiciouse_email_pressed():
	$Explanation.show()
	$Explanation/Name.text = tr("Email Phishing")
	$Explanation/Explanation.text =tr("Phishing is an attack in which an attacker writes a message to a target by email, chat, or other text communication tool and tries to receive something interesting. The attacker's target could be personal information, bank account information, or installing on target workstation malware, which collects user data. In this example, the user received a message from another worker who needed information about a record that he worked on ealier but could not receive access to it because his account was locked.")
	$Explanation/Mitigation.text = tr("Here are instructions on what to do to not find yourself in such a situation:
...
func _on_vpn_pressed():
	$Explanation.show()
	$Explanation/Name.text = tr("VPN phishing")
	$Explanation/Explanation.text = tr("Type of social engineering attack when attacker gives target address by which he accesses to the private network. When the target is connected, the attacker could see data traffic from a user. This could cause loss of your private data, such as passwords from accounts, private photo or video leaks, and other sensitive information.")
	$Explanation/Mitigation.text = tr("Here are instructions on what to do to not find yourself in such a situation:
...
func _on_wrong_path_pressed():
	$Explanation.show()
	$Explanation/Name.text = tr("Pharming with known webpage")
	$Explanation/Explanation.text = tr("Pharming is an attack in which the attacker creates a copy of the original website or profile and uses it to collect data from the target, impersonating itself as legitimate. In this example, we have an original link of the company where the content was changed so that hacker collect information from users.")
	$Explanation/Mitigation.text = tr("Here are instructions on what to do to not find yourself in such a situation:
...
func _on_malisiouse_prize_pressed():
	$Explanation.show()
	$Explanation/Name.text = tr("Pharming with prize")
...
func _on_maliciouse_link_pressed():
	$Explanation.show()
	$Explanation/Name.text = tr("Pharming with malisious link")
...
func _on_data_protection_pressed():
	$Explanation.show()
	$Explanation/Name.text = tr("Phishig of personal data")
